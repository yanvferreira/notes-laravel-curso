        ...
        <div class="row justify-content-center">
            <div class="col">
                ...
                <form action="{{ route('editNoteSubmit') }}" method="post">
                    @csrf
                    <input type="hidden" name="nota_id" value="{{ Crypt::encrypt($nota->id) }}">
                    <div class="row mt-3">
                        <div class="col">
                            <div class="mb-3">
                                <label class="form-label">Título da Nota</label>
                                <input type="text" class="form-control bg-primary text-white" name="text_title" value="{{ old('text_title', $nota->title) }}">
                                {{-- show errors --}}
                                @error('text_title')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control bg-primary text-white" name="text_note" rows="5">{{ old('text_note', $nota->text) }}</textarea>
                                @error('text_note')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
